...
public class SDcppParameterBuilder
{
    ...
    public void AddBasicParameters(Dictionary<string, object> parameters, T2IParamInput input, bool isFluxModel)
    {
        ...
        if (input.TryGet(T2IParamTypes.Prompt, out string prompt))
        {
            ...
            // Check for LoRA in prompt
            if (prompt.Contains("<lora:"))
            {
                string loraFolder = Program.ServerSettings.Paths.SDLoraFolder.Split(';')[0];
                string loraDir = Utilities.CombinePathWithAbsolute(
                    Program.ServerSettings.Paths.ActualModelRoot, loraFolder);
        ...
        // Scheduler
        if (SDcppExtension.SchedulerParam is not null &&
            input.TryGet(SDcppExtension.SchedulerParam, out string scheduler) &&
            !string.IsNullOrEmpty(scheduler) && scheduler is not "default")
        {
        ...
        bool isFluxModel, bool isSD3Model, bool isZImageModel)
    {
        ...
        else if (mainModel is not null)
        {
            ...
            // Optional VAE for standard models
            if (input.TryGet(T2IParamTypes.VAE, out T2IModel vaeModel) &&
                vaeModel is not null && vaeModel.Name is not "(none)")
            {
    ...
    public void AddVAE(Dictionary<string, object> parameters, T2IParamInput input, bool isFluxModel)
    {
        ...
        // Auto-detect Flux VAE
        T2IModelHandler vaeModelSet = Program.T2IModelSets["VAE"];
        T2IModel defaultVae = vaeModelSet.Models.Values.FirstOrDefault(m =>
            ...
            (m.Name.Contains("flux", StringComparison.OrdinalIgnoreCase) &&
             m.Name.Contains("ae", StringComparison.OrdinalIgnoreCase)));
        ...
        else
        {
            ...
            string vaePath = SDcppModelManager.EnsureModelExists("VAE", "Flux/ae.safetensors",
    ...
    public void AddSD3Encoders(Dictionary<string, object> parameters, T2IParamInput input)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                string clipGPath = SDcppModelManager.EnsureModelExists("Clip", "clip_g.safetensors",
    ...
    public void AddCLIPEncoders(Dictionary<string, object> parameters, T2IParamInput input)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                string clipLPath = SDcppModelManager.EnsureModelExists("Clip", "clip_l.safetensors",
    ...
    public void AddT5XXL(Dictionary<string, object> parameters, T2IParamInput input)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                string t5Path = SDcppModelManager.EnsureModelExists("Clip", "t5xxl_fp8_e4m3fn.safetensors",
    ...
    public void AddQwenLLM(Dictionary<string, object> parameters, T2IParamInput input)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                string qwenPath = SDcppModelManager.EnsureModelExists("Clip", "qwen_3_4b.safetensors",
        ...
        bool isFluxModel, bool isSD3Model, bool isZImageModel)
    {
        ...
        if (isFluxModel)
        {
            if (!parameters.ContainsKey("vae"))
                missing.Add("VAE (ae.safetensors)");
            if (!parameters.ContainsKey("clip_l"))
                missing.Add("CLIP-L (clip_l.safetensors)");
            if (!parameters.ContainsKey("t5xxl"))
                missing.Add("T5-XXL (t5xxl_fp16.safetensors or t5xxl_fp8_e4m3fn.safetensors)");

            if (missing.Count > 0)
            {
                throw new InvalidOperationException(
                    $"Flux models require additional component files that are not installed.\n\n" +
                    $"Missing components:\n  • {string.Join("\n  • ", missing)}\n\n" +
        ...
        {
            if (!parameters.ContainsKey("clip_g"))
                missing.Add("CLIP-G (clip_g.safetensors)");
            if (!parameters.ContainsKey("clip_l"))
                missing.Add("CLIP-L (clip_l.safetensors)");
            if (!parameters.ContainsKey("t5xxl"))
                missing.Add("T5-XXL (t5xxl_fp16.safetensors)");

            if (missing.Count > 0)
            {
                throw new InvalidOperationException(
                    $"SD3 models require additional component files that are not installed.\n\n" +
                    $"Missing components:\n  • {string.Join("\n  • ", missing)}");
            }
        ...
        {
            if (!parameters.ContainsKey("vae"))
                missing.Add("VAE (flux-ae.safetensors)");
            if (!parameters.ContainsKey("llm"))
                missing.Add("Qwen LLM (qwen_3_4b.safetensors)");

            if (missing.Count > 0)
            {
                throw new InvalidOperationException(
                    $"Z-Image models require additional component files that are not installed.\n\n" +
                    $"Missing components:\n  • {string.Join("\n  • ", missing)}");
            }
    ...
    public void AddImageParameters(Dictionary<string, object> parameters, T2IParamInput input, string outputDir)
    {
        ...
        // ControlNet support (SD.cpp supports single ControlNet)
        for (int i = 0; i < T2IParamTypes.Controlnets.Length; i++)
        {
            T2IParamTypes.ControlNetParamHolder cn = T2IParamTypes.Controlnets[i];

            if (input.TryGet(cn.Model, out T2IModel cnModel) &&
                cnModel is not null && cnModel.Name is not "(None)")
            {
    ...
    public void AddAdvancedParameters(Dictionary<string, object> parameters, T2IParamInput input)
    {
        ...
        if (SDcppExtension.TAESDParam is not null &&
            input.TryGet(SDcppExtension.TAESDParam, out T2IModel taesdModel) &&
            taesdModel is not null && taesdModel.Name is not "(None)")
        {
        ...
        // ESRGAN upscaling
        if (SDcppExtension.UpscaleModelParam is not null &&
            input.TryGet(SDcppExtension.UpscaleModelParam, out T2IModel upscaleModel) &&
            upscaleModel is not null && upscaleModel.Name is not "(None)")
        {
            parameters["upscale_model"] = upscaleModel.RawFilePath;
            Logs.Info($"[SDcpp] Using ESRGAN upscale model: {upscaleModel.Name}");

            if (SDcppExtension.UpscaleRepeatsParam is not null &&
                input.TryGet(SDcppExtension.UpscaleRepeatsParam, out int upscaleRepeats) &&
                upscaleRepeats > 1)
            {
        ...
        // Color projection
        if (SDcppExtension.ColorProjectionParam is not null &&
            input.TryGet(SDcppExtension.ColorProjectionParam, out bool colorProjection) &&
            colorProjection)
        {
    ...
    public void AddVideoParameters(Dictionary<string, object> parameters, T2IParamInput input)
    {
        ...
        // Wan 2.2 dual-model support
        if (CurrentModelArchitecture is "wan-2.2")
        {
            if (input.TryGet(T2IParamTypes.VideoSwapModel, out T2IModel swapModel) &&
                swapModel is not null && swapModel.Name is not "(None)")
            {
